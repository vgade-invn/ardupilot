...
static Vector3d guided_pos_target_cm;       // position target (used by posvel controller only)
...
bool ModeGuided::set_destination_posvel(const Vector3f& destination, const Vector3f& velocity, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
    ...
    guided_pos_target_cm = destination.todouble();
    guided_vel_target_cms = velocity;
...
void ModeGuided::posvel_control_run()
{
    ...
    guided_pos_target_cm += (guided_vel_target_cms * pos_control->get_dt()).todouble();

    // send position and velocity targets to position controller
    pos_control->input_pos_vel_accel_xy(guided_pos_target_cm, guided_vel_target_cms, Vector3f());
    Vector3f target_f = guided_vel_target_cms.tofloat();
    pos_control->input_pos_vel_accel_z(target_f, guided_vel_target_cms, Vector3f());
    guided_pos_target_cm.z = target_f.z;
