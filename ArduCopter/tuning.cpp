...
/*
  reload a parameter
 */
void AP_Tuning_Copter::reload_value(uint8_t parm)
{
    switch(parm) {
    // special handling of dual-parameters
    case TUNING_RATE_ROLL_KPI:
        reload_value(TUNING_RATE_ROLL_KP);
        reload_value(TUNING_RATE_ROLL_KI);
        break;
    case TUNING_RATE_PITCH_KPI:
        reload_value(TUNING_RATE_PITCH_KP);
        reload_value(TUNING_RATE_PITCH_KI);
        break;
    default:
        AP_Float *f = get_param_pointer(parm);
        if (f != nullptr) {
