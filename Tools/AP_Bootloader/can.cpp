...
#define FW_LOAD_DEBUG 1
#if FW_LOAD_DEBUG
#define debug(fmt, args ...)  do { uprintf(fmt, ## args); } while(0)
#else
...
static void handle_file_read_response(CanardInstance* ins, CanardRxTransfer* transfer)
{
    ...
    if (len < UAVCAN_PROTOCOL_FILE_READ_RESPONSE_DATA_MAX_LENGTH) {
        ...
        if (can_check_firmware()) {
            debug("flash check OK\n");
            jump_to_app();
        } else {
            debug("flash check failed %u\n", node_status.vendor_specific_status_code);
        }
    }
