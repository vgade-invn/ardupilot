...
bool LogReader::handle_log_format_msg(const struct log_Format &f) 
{
	...
	if (streq(name, "PARM")) {
	    ...
	    msgparser[f.type] = new LR_MsgHandler_NKF1(formats[f.type], ekf2, ekf3);
	} else if (streq(name, "XKF1")) {
	    msgparser[f.type] = new LR_MsgHandler_XKF1(formats[f.type], ekf2, ekf3);
	    ...
	    msgparser[f.type] = new LR_MsgHandler_RISJ(formats[f.type]);
