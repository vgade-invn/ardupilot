...
void NavEKF3_core::CovarianceInit()
{
    ...
    // quaternions
    P[3][3] = P[2][2] = P[1][1] = P[0][0] = sq(0.1f);
...
void NavEKF3_core::CovariancePrediction(Vector3f *rotVarVecPtr)
{
    ...
    const float dvx = imuDataDelayed.delVel.x;
    const float dvy = imuDataDelayed.delVel.y;
    const float dvz = imuDataDelayed.delVel.z;
    const float dax = imuDataDelayed.delAng.x;
    const float day = imuDataDelayed.delAng.y;
    const float daz = imuDataDelayed.delAng.z;
    ...
    const float dax_b = stateStruct.gyro_bias.x;
    const float day_b = stateStruct.gyro_bias.y;
    const float daz_b = stateStruct.gyro_bias.z;
    const float dvx_b = stateStruct.accel_bias.x;
    const float dvy_b = stateStruct.accel_bias.y;
    const float dvz_b = stateStruct.accel_bias.z;
    ...
    const float dvxVar = sq(dt*_accNoise); // X axis delta velocity variance noise (m/s)^2
    const float dvyVar = dvxVar;           // Y axis delta velocity variance noise (m/s)^2
    const float dvzVar = dvxVar;           // Z axis delta velocity variance noise (m/s)^2
