...
float ComplementaryFilter::apply(float low_freq, float high_freq, uint32_t time_us)
{
    ...
    lp.set_cutoff_frequency(sample_freq, cutoff_freq);
    hp.set_cutoff_frequency(sample_freq, cutoff_freq);

    float hp_out = hp.apply(high_freq);
    float lp_out = lp.apply(low_freq);
