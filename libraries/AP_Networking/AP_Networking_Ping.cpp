...
#ifndef AP_NETWORKING_PING_DEFAULT_PORT
#define AP_NETWORKING_PING_DEFAULT_PORT   5555
#endif

const AP_Param::GroupInfo AP_Networking_Ping::var_info[] = {

    AP_GROUPINFO("DST_ADDR0", 2,  AP_Networking_Ping,    _params.ip[0],   255),
    AP_GROUPINFO("DST_ADDR1", 3,  AP_Networking_Ping,    _params.ip[1],   255),
    AP_GROUPINFO("DST_ADDR2", 4,  AP_Networking_Ping,    _params.ip[2],   255),
    AP_GROUPINFO("DST_ADDR3", 5,  AP_Networking_Ping,    _params.ip[3],   255),

    // @Param: DURATION
    // @DisplayName: Ping duration
    // @Description: Ping duration in seconds. Setting the value > 0 will start the pinging. To protect the network, this value resets to zero on boot and after the Ping duration has elapsed. Set to -1 to ping forever until next boot.
    // @Units: s
    // @User: Standard
    AP_GROUPINFO("DURATION", 9, AP_Networking_Ping, _params.duration_seconds, 0),
...
/// Constructor
AP_Networking_Ping::AP_Networking_Ping(AP_Networking &front, AP_Networking::AP_Networking_State &state,  AP_Networking_Params &params) :
...
void AP_Networking_Ping::update()
{
    ...
    if (_stats.start_ms == 0) {
        ...
        GCS_SEND_TEXT(MAV_SEVERITY_INFO,"NET: Ping START, Duration %.2fs", (double)_params.duration_seconds.get());
    }
...
// void AP_Networking_Ping::send_data()
// {
...
// bool AP_Networking_Ping::report_stats()
// {
