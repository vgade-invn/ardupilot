...
#define debug_uavcan(level_debug, fmt, args...) do { if ((level_debug) <= AP::can().get_debug_level_driver(_driver_num)) { printf(fmt, ##args); }} while (0)
...
void AP_UAVCAN::init(uint8_t driver_num)
{
    if (_initialized) {
        return;
    }
    ...
    AP_HAL::CANManager* can_mgr = hal.can_mgr[driver_num];
    if (can_mgr == nullptr) {
        return;
    }
    ...
    char ndname[20];
    snprintf(ndname, sizeof(ndname), "org.ardupilot:%u", driver_num);
    ...
    const int gnss_fix_start_res = gnss_fix->start(gnss_fix_cb_arr[driver_num]);
    ...
    const int gnss_aux_start_res = gnss_aux->start(gnss_aux_cb_arr[driver_num]);
    ...
    const int magnetic_start_res = magnetic->start(magnetic_cb_arr[driver_num]);
    ...
    const int magnetic_start_res_2 = magnetic2->start(magnetic_cb_2_arr[driver_num]);
    ...
    const int air_data_sp_start_res = air_data_sp->start(air_data_sp_cb_arr[driver_num]);
    ...
    const int air_data_st_start_res = air_data_st->start(air_data_st_cb_arr[driver_num]);
    ...
    const int battery_info_start_res = battery_info_st->start(battery_info_st_cb_arr[driver_num]);
    if (battery_info_start_res < 0) {
        debug_uavcan(1, "UAVCAN BatteryInfo subscriber start problem\n\r");
        return;
    }

    act_out_array[driver_num] = new uavcan::Publisher<uavcan::equipment::actuator::ArrayCommand>(*node);
    act_out_array[driver_num]->setTxTimeout(uavcan::MonotonicDuration::fromMSec(2));
    act_out_array[driver_num]->setPriority(uavcan::TransferPriority::OneLowerThanHighest);

    esc_raw[driver_num] = new uavcan::Publisher<uavcan::equipment::esc::RawCommand>(*node);
    esc_raw[driver_num]->setTxTimeout(uavcan::MonotonicDuration::fromMSec(2));
    esc_raw[driver_num]->setPriority(uavcan::TransferPriority::OneLowerThanHighest);

    rgb_led[driver_num] = new uavcan::Publisher<uavcan::equipment::indication::LightsCommand>(*node);
    rgb_led[driver_num]->setTxTimeout(uavcan::MonotonicDuration::fromMSec(20));
    rgb_led[driver_num]->setPriority(uavcan::TransferPriority::OneHigherThanLowest);
    ...
    snprintf(_thread_name, sizeof(_thread_name), "uavcan_%u", driver_num);
...
void AP_UAVCAN::SRV_send_servos(void)
{
    ...
    do {
        ...
        if (i > 0) {
            act_out_array[_driver_num]->broadcast(msg);
...
void AP_UAVCAN::SRV_send_esc(void)
{
    ...
    if (active_esc_num > 0) {
        ...
        esc_raw[_driver_num]->broadcast(esc_msg);
    }
}
...
void AP_UAVCAN::led_out_send()
{
    if (_led_conf.broadcast_enabled && ((AP_HAL::micros64() - _led_conf.last_update) > (AP_UAVCAN_LED_DELAY_MILLISECONDS * 1000))) {
        ...
        rgb_led[_driver_num]->broadcast(msg);
...
{
    if (hal.can_mgr[_driver_num] != nullptr) {
        if (hal.can_mgr[_driver_num]->is_initialized() == false) {
            return nullptr;
        } else {
            return hal.can_mgr[_driver_num]->get_driver();
        }
    }
    return nullptr;
}
