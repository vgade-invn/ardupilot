...
public:
    ...
    // Return uavcan from @driver_num or nullptr if it's not ready or doesn't exist
    static AP_UAVCAN *get_uavcan(uint8_t driver_num);

    void init(uint8_t driver_num) override;
    ...
    protected:
        typedef void* (*Registry)(AP_UAVCAN*, uint8_t, const RegistryBinder&);
        AP_UAVCAN* _uc;
        Registry _ffunc;
    public:
        RegistryBinder()
