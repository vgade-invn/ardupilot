...
const AP_Param::GroupInfo AP_MotorsHeli_Compound::var_info[] = {
    ...
    // @User: Standard
    // @Increment: 1
    AP_GROUPINFO("SV1_POS", 1, AP_MotorsHeli_Compound, _servo1_pos, AP_MOTORS_HELI_COMPOUND_SERVO1_POS),
    ...
    // @User: Standard
    // @Increment: 1
    AP_GROUPINFO("SV2_POS", 2, AP_MotorsHeli_Compound, _servo2_pos, AP_MOTORS_HELI_COMPOUND_SERVO2_POS),
    ...
    // @User: Standard
    // @Increment: 1
    AP_GROUPINFO("SV3_POS", 3, AP_MotorsHeli_Compound, _servo3_pos, AP_MOTORS_HELI_COMPOUND_SERVO3_POS),
    ...
    // @User: Standard
    AP_GROUPINFO("TAIL_TYPE", 4, AP_MotorsHeli_Compound, _tail_type, AP_MOTORS_HELI_COMPOUND_TAILTYPE_SERVO),
    ...
    // @User: Standard
    AP_GROUPINFO("SWASH_TYPE", 5, AP_MotorsHeli_Compound, _swash_type, AP_MOTORS_HELI_COMPOUND_SWASH_CCPM),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("GYR_GAIN", 6, AP_MotorsHeli_Compound, _ext_gyro_gain_std, AP_MOTORS_HELI_COMPOUND_EXT_GYRO_GAIN),
    ...
    // @User: Advanced
    // @Increment: 1
    AP_GROUPINFO("PHANG", 7, AP_MotorsHeli_Compound, _phase_angle, 0),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("COLYAW", 8,  AP_MotorsHeli_Compound, _collective_yaw_effect, 0),
    ...
    // @User: Standard
    AP_GROUPINFO("FLYBAR_MODE", 9, AP_MotorsHeli_Compound, _flybar_mode, AP_MOTORS_HELI_NOFLYBAR),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("GYR_GAIN_ACRO", 11, AP_MotorsHeli_Compound,  _ext_gyro_gain_acro, 0),
    ...
    // @Range: 0 2000
    // @User: Standard
    AP_GROUPINFO("RSC_PWM_MIN", 16, AP_MotorsHeli_Compound, _main_rotor._pwm_min, 1000),
    ...
    // @Range: 0 2000
    // @User: Standard
    AP_GROUPINFO("RSC_PWM_MAX", 17, AP_MotorsHeli_Compound, _main_rotor._pwm_max, 2000),
    ...
    // @User: Standard
    AP_GROUPINFO("RSC_PWM_REV", 18, AP_MotorsHeli_Compound, _main_rotor._pwm_rev, 1),
...
void AP_MotorsHeli_Compound::enable()
{
    ...
    rc_enable_ch(AP_MOTORS_MOT_4);    // yaw servo 1
    rc_enable_ch(AP_MOTORS_MOT_5);    // yaw servo 2
    rc_enable_ch(AP_MOTORS_HELI_COMPOUND_AUX);                                 // output for gyro gain or direct drive variable pitch tail motor
    rc_enable_ch(AP_MOTORS_HELI_COMPOUND_RSC);                                 // output for main rotor esc
}

// init_outputs - initialise Servo/PWM ranges and endpoints
bool AP_MotorsHeli_Compound::init_outputs()
{
    if (!_flags.initialised_ok) {
        ...
        _yaw_servo_1 = SRV_Channels::get_channel_for(SRV_Channel::k_motor4, CH_4);
        _yaw_servo_2 = SRV_Channels::get_channel_for(SRV_Channel::k_motor5, CH_5);
        _servo_aux = SRV_Channels::get_channel_for(SRV_Channel::k_motor7, CH_7);
        if (!_swash_servo_1 || !_swash_servo_2 || !_swash_servo_3 || !_yaw_servo_1 || !_yaw_servo_2 || !_servo_aux) {
            return false;
        }
    }
    ...
    _yaw_servo_1->set_angle(4500);
    _yaw_servo_2->set_angle(4500);
...
void AP_MotorsHeli_Compound::output_test(uint8_t motor_seq, int16_t pwm)
{
    ...
    switch (motor_seq) {
        ...
        case 6:
            // main rotor
            rc_write(AP_MOTORS_HELI_COMPOUND_RSC, pwm);
            break;
        default:
            // do nothing
            break;
...
void AP_MotorsHeli_Compound::set_desired_rotor_speed(float desired_speed)
{
...
void AP_MotorsHeli_Compound::calculate_armed_scalars()
{
...
// calculate_scalars - recalculates various scalers used.
void AP_MotorsHeli_Compound::calculate_scalars()
{
    ...
    if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPITCH) {
        _tail_rotor.set_control_mode(ROTOR_CONTROL_MODE_SPEED_SETPOINT);
        _tail_rotor.set_ramp_time(AP_MOTORS_HELI_COMPOUND_DDVPT_RAMP_TIME);
        _tail_rotor.set_runup_time(AP_MOTORS_HELI_COMPOUND_DDVPT_RUNUP_TIME);
...
void AP_MotorsHeli_Compound::calculate_roll_pitch_collective_factors()
{
    if (_swash_type == AP_MOTORS_HELI_COMPOUND_SWASH_CCPM) {                     //CCPM Swashplate, perform control mixing
...
uint16_t AP_MotorsHeli_Compound::get_motor_mask()
{
    ...
    return (1U << AP_MOTORS_MOT_1 | 1U << AP_MOTORS_MOT_2 | 1U << AP_MOTORS_MOT_3 | 1U << AP_MOTORS_MOT_4 | 1U << AP_MOTORS_MOT_5 | 1U << AP_MOTORS_HELI_COMPOUND_AUX | 1U << AP_MOTORS_HELI_COMPOUND_RSC);
}

// update_motor_controls - sends commands to motor controllers
void AP_MotorsHeli_Compound::update_motor_control(RotorControlState state)
{
...
void AP_MotorsHeli_Compound::move_actuators(float roll_out, float pitch_out, float coll_in, float yaw_out)
{
    ...
    if (_servo_mode == SERVO_CONTROL_MODE_AUTOMATED) {
        ...
        if ((_main_rotor.get_control_output() > _main_rotor.get_idle_output()) && _tail_type != AP_MOTORS_HELI_COMPOUND_TAILTYPE_SERVO_EXTGYRO) {
            // sanity check collective_yaw_effect
            _collective_yaw_effect = constrain_float(_collective_yaw_effect, -AP_MOTORS_HELI_COMPOUND_COLYAW_RANGE, AP_MOTORS_HELI_COMPOUND_COLYAW_RANGE);
    ...
    if (_swash_type == AP_MOTORS_HELI_COMPOUND_SWASH_H1) {
        servo1_out += 0.5f;
        servo2_out += 0.5f;
    }
...
void AP_MotorsHeli_Compound::move_yaw(float yaw_out)
{
    ...
    } else if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_FIXEDPITCH && _main_rotor.get_desired_speed() > 0.0f) {
...
void AP_MotorsHeli_Compound::write_aux(float servo_out)
{
    rc_write(AP_MOTORS_HELI_COMPOUND_AUX, calc_pwm_output_0to1(servo_out, _servo_aux));
}

// servo_test - move servos through full range of movement
void AP_MotorsHeli_Compound::servo_test()
{
...
bool AP_MotorsHeli_Compound::parameter_check(bool display_msg) const
{
