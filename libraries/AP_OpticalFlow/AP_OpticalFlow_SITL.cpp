...
void AP_OpticalFlow_SITL::update(void)
{
    ...
    state.flowRate.x = (-relVelSensor.y/range + gyro.x + rand_normal(_sitl->flow_noise)) * flowScaleFactorX;
    state.flowRate.y =  (relVelSensor.x/range + gyro.y + rand_normal(_sitl->flow_noise)) * flowScaleFactorY;
