...
public:
    ...
    Vector2f update_all(double &target_x, double &target_y, const Vector2f &measurement, bool &limit) WARN_IF_UNUSED;

    // set target and measured inputs to P controller and calculate outputs
    // measurement is provided as 3-axis vector but only x and y are used
    Vector2f update_all(double &target_x, double &target_y, const Vector3f &measurement, bool &limit) WARN_IF_UNUSED {
