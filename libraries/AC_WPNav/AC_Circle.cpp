...
void AC_Circle::init(const Vector3d& center, bool terrain_alt)
{
    _center = center;
...
void AC_Circle::init()
{
    ...
    // get stopping point
    const Vector3d& stopping_point = _pos_control.get_pos_target_cm();
...
bool AC_Circle::update(float climb_rate_cms)
{
    ...
    if(_terrain_alt) {
        Vector3f targetf = target.tofloat();
        _pos_control.input_pos_vel_accel_z(targetf, zero, zero);
        target.z = targetf.z;
    } else {
