...
// button held down for 3 intervals will cause a power off event
#define BATTMONITOR_SMBUS_SUI_BUTTON_DEBOUNCE 3
...
void AP_BattMonitor_SMBus_SUI::read_cell_voltages()
{
    ...
    if (cell_count >= SUI_MAX_CELL_READ) {
        ...
        if (read_block(BATTMONITOR_SMBUS_VOLTAGE, (uint8_t *)&total_mv, sizeof(total_mv)) &&
            total_mv >= pack_voltage_mv) {
            for (uint8_t i = SUI_MAX_CELL_READ; i < cell_count; i++) {
                _state.cell_voltages.cells[i] = (total_mv - pack_voltage_mv) / (cell_count - SUI_MAX_CELL_READ);
            }
