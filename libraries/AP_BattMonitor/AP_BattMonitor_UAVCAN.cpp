...
AP_BattMonitor_UAVCAN* AP_BattMonitor_UAVCAN::get_uavcan_backend(AP_UAVCAN* ap_uavcan, uint8_t node_id)
{
    ...
    for (uint8_t i = 0; i < AP::battery()._num_instances; i++) {
        ...
        if (AP::battery().drivers[i]->get_uavcan_manager() == ap_uavcan && AP::battery().drivers[i]->get_uavcan_node() == node_id) {
            return (AP_BattMonitor_UAVCAN*)AP::battery().drivers[i];
        }
    }
    ...
    for (uint8_t i = 0; i < AP::battery()._num_instances; i++) {
        if (AP::battery().drivers[i] != nullptr && AP::battery().get_type(i) == AP_BattMonitor_Params::BattMonitor_TYPE_UAVCAN_BatteryInfo) {
            ...
            batmon->init();
            debug_bm_uavcan(2, ap_uavcan->get_driver_num(),"Registered BattMonitor Node %d on Bus %d\n", node_id, ap_uavcan->get_driver_num());
            return batmon;
        }
    }
    return nullptr;
    ...
    _type(type)
{
    ...
    _sem_battmon = hal.util->new_semaphore();
}
...
void AP_BattMonitor_UAVCAN::read()
{
    ...
    if (_sem_battmon->take(HAL_SEMAPHORE_BLOCK_FOREVER)) {
