...
struct AP_BattMonitor_FuelFlow::IrqState AP_BattMonitor_FuelFlow::irq_state;
    ...
    AP_BattMonitor_Backend(mon, mon_state, params)
{
    // always healthy
    _state.healthy = true;
    printf("started FuelFlow\n");
}

/*
  handle interrupt on an instance
 */
void AP_BattMonitor_FuelFlow::irq_handler(void)
{
    uint32_t timestamp = AP_HAL::micros();
    if (irq_state.last_pulse_us == 0) {
        irq_state.last_pulse_us = timestamp;
        return;
    }
    irq_state.pulse_count++;
    irq_state.total_us += (timestamp - irq_state.last_pulse_us);
    irq_state.last_pulse_us = timestamp;
}
...
void AP_BattMonitor_FuelFlow::read()
{
    ...
    if (last_pin != pin) {
        ...
        if (last_pin != -1) {
            hal.gpio->attach_interrupt(last_pin,nullptr, 0);
        }
        // attach to new pin
        last_pin = pin;
        if (last_pin > 0) {
    ...
    // map consumed_wh using fixed voltage
    _state.consumed_wh += litres * 1000 * _state.voltage;
