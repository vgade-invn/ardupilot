...
const AnalogIn::pin_info AnalogIn::pin_config[] = { HAL_ANALOG_PINS };
    ...
    #define HAL_MCU_MONITORING_PINS {ADC3_VBAT4_CHAN, 3.30/4096}, {ADC3_VSENSE_CHAN, 3.30/4096}, {ADC3_VREFINT_CHAN, 3.30/4096}
#else
 ...
 */
uint8_t AnalogIn::get_pin_channel(uint8_t index, uint8_t pin)
{
    switch(index) {
    case 0:
        return pin_config[pin].channel;
#if defined(HAL_ANALOG2_PINS)
    case 1:
        return pin_config_2[pin].channel;
#endif
#if defined(HAL_ANALOG3_PINS)
    case 2:
        return pin_config_3[pin].channel;
#endif
    };
    return 255;
}

uint8_t AnalogIn::get_pin_channel_with_offset(uint8_t index, uint8_t pin)
{
    return get_pin_channel(index, pin) + index*HAL_MAX_ANALOG_IN_CHANNELS;
}
/*
  return scaling
 */
float AnalogIn::get_pin_scaling(uint8_t index, uint8_t pin)
{
    switch(index) {
    case 0:
        return pin_config[pin].scaling;
#if defined(HAL_ANALOG2_PINS)
    case 1:
        return pin_config_2[pin].scaling;
#endif
#if defined(HAL_ANALOG3_PINS)
    case 2:
        return pin_config_3[pin].scaling;
#endif
    };
    return 0;
}
...
void AnalogIn::adccallback(ADCDriver *adcp)
{
    ...
    uint8_t real_buff_length;
#if STM32_ADC_DUAL_MODE
    if (index == 0) {
        real_buff_length = MAX(num_grp_channels, get_num_grp_channels(1));
        stm32_cacheBufferInvalidate(buffer, sizeof(uint16_t)*ADC_DMA_BUF_DEPTH*real_buff_length*2);
    } else
#endif
    {
        real_buff_length = num_grp_channels;
    ...
    for (uint8_t i = 0; i < ADC_DMA_BUF_DEPTH; i++) {
        for (uint8_t j = 0; j < real_buff_length; j++) {
            if (j < num_grp_channels) {
...
#if STM32_ADC_DUAL_MODE
    if (index == 0) {
        buffer = (uint16_t*)samples[0] + 1;
        num_grp_channels = get_num_grp_channels(1);
        // read the samples from the second ADC
        for (uint8_t i = 0; i < ADC_DMA_BUF_DEPTH; i++) {
            for (uint8_t j = 0; j < real_buff_length; j++) {
                if (j < num_grp_channels) {
...
void AnalogIn::setup_adc(uint8_t index)
{
    ...
    if (index == 0) {
        ...
        num_grp_channels = MAX(num_grp_channels, get_num_grp_channels(1)) * 2;
        samples[0] = (adcsample_t *)hal.util->malloc_type(sizeof(uint16_t)*ADC_DMA_BUF_DEPTH*num_grp_channels, AP_HAL::Util::MEM_DMA_SAFE);
        sample_sum[0] = (uint32_t *)malloc(sizeof(uint32_t)*get_num_grp_channels(0));
        sample_sum[1] = (uint32_t *)malloc(sizeof(uint32_t)*get_num_grp_channels(1));        
    } else {
    ...
    sample_sum[index] = (uint32_t *)malloc(sizeof(uint32_t)*num_grp_channels);
#endif
    if (samples[index] == nullptr || sample_sum[index] == nullptr) {
        // panic if we can't allocate memory
        AP_HAL::panic("Failed to allocate ADC memory");
    }
...
#if STM32_ADC_DUAL_MODE
        if (chan == 255) {
            // if ADC2 has higher number of channels we
            // mirror those channels for ADC1, and vice versa
            chan = get_pin_channel(1, i);
        }
#else
        if (chan == 255) {
            continue;
...
void AnalogIn::_timer_tick(void)
{
    ...
    for (uint8_t index=0; index < HAL_NUM_ANALOG_INPUTS; index++) {
        ...
        for (uint8_t i=0; i < num_grp_channels; i++) {
            ...
            if (get_pin_channel_with_offset(index, i) == ANALOG_VCC_5V_PIN) {
            ...
            if (get_pin_channel_with_offset(index, i) == FMU_SERVORAIL_ADC_CHAN) {
        ...
        for (uint8_t i=0; i<num_grp_channels; i++) {
            ...
            for (uint8_t j=0; j < ANALOG_MAX_CHANNELS; j++) {
                ...
                if (c != nullptr) {
                    if (get_pin_channel_with_offset(index, i) == c->_pin) {
                        // add a value
