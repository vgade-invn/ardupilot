...
uint8_t handler_offset_to_pin[6] = {};
...
void PX4GPIO::irq_handler(uint8_t handler)
{
    ...
    const uint8_t pin = handler_offset_to_pin[handler];
                               ...
                               INTERRUPT_TRIGGER_TYPE mode)
{
    ...
    if (pin_handlers[handler_offset]) {
        stm32_gpiosetevent(gpio, false, false, false, nullptr);
        pin_handlers[handler_offset].invalidate();
        gcs().send_text(MAV_SEVERITY_WARNING, "Removed handler @%u", handler_offset);
    }
    ...
    // attach a distict interrupt handler for each different GPIO
    ...
    handler_offset_to_pin[handler_offset] = pin;

    gcs().send_text(MAV_SEVERITY_WARNING, "Attached handler @%u", handler_offset);
