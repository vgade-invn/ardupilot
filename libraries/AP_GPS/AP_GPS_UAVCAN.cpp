...
void AP_GPS_UAVCAN::subscribe_gps_uavcan_messages(AP_UAVCAN* ap_uavcan)
{
    ...
    static uavcan::Subscriber<uavcan::equipment::gnss::Fix, FixCb> *gnss_fix;
...
AP_GPS_UAVCAN* AP_GPS_UAVCAN::get_uavcan_backend(AP_UAVCAN* ap_uavcan, uint8_t node_id)
{
    ...
    for (uint8_t i = 0; i < GPS_MAX_RECEIVERS; i++) {
        ...
        if (_frontend.drivers[i]->get_uavcan_manager() == ap_uavcan && _frontend.drivers[i]->get_uavcan_node() == node_id) {
            return (AP_GPS_UAVCAN*)_frontend.drivers[i];
        }
    }
...
AP_GPS_Backend* AP_GPS_UAVCAN::allocate_detected_modules(AP_GPS &_gps, AP_GPS::GPS_State &_state)
{
    if (!take_registry()) {
        return nullptr;
    }
    ...
    for (uint8_t i = 0; i < GPS_MAX_RECEIVERS; i++) {
        if (_detected_modules[i].ap_uavcan != nullptr) {
            ...
            if (backend == nullptr) {
                debug_gps_uavcan(2, _detected_modules[i].ap_uavcan->get_driver_num(), "Failed to register UAVCAN GPS Node %d on Bus %d\n", _detected_modules[i].node_id,
                _detected_modules[i].ap_uavcan->get_driver_num());
            } else {
                backend->ap_uavcan = _detected_modules[i].ap_uavcan;
                backend->node_id = _detected_modules[i].node_id;
                debug_gps_uavcan(2, _detected_modules[i].ap_uavcan->get_driver_num(), "Registered UAVCAN GPS Node %d on Bus %d\n", _detected_modules[i].node_id, _detected_modules[i].ap_uavcan->get_driver_num());
            }
