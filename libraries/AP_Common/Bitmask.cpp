...
_Bitmask::_Bitmask(uint16_t num_bits, uint32_t *_bits) :
    bits(_bits),
    numbits(num_bits),
...
void _Bitmask::set(uint16_t bit)
{
...
bool _Bitmask::get(uint16_t bit) const
{
    uint16_t word = bit/32;
    uint8_t ofs = bit & 0x1f;
...
// copy one bitmask to another
_Bitmask &_Bitmask::operator=(const _Bitmask&other)
{
    memcpy(bits, other.bits, sizeof(bits[0])*other.numwords);
    return *this;
}

// check equality of two bitmasks
bool _Bitmask::operator==(const _Bitmask&other)
{
...
// check inequality of two bitmasks
bool _Bitmask::operator!=(const _Bitmask&other)
{
    return !(*this == other);
}
...
void _Bitmask::clear(uint16_t bit)
{
    uint16_t word = bit/32;
    uint8_t ofs = bit & 0x1f;
    bits[word] &= ~(1U << ofs);
...
void _Bitmask::setonoff(uint16_t bit, bool onoff)
{
    if (onoff) {
        set(bit);
    } else {
...
// clear all bits
void _Bitmask::clearall(void)
{
    memset(bits, 0, numwords*sizeof(bits[0]));
}

// return true if all bits are clear
bool _Bitmask::empty(void) const
{
...
int16_t _Bitmask::first_set() const
{
    for (uint16_t i=0; i<numwords; i++) {
        if (bits[i] == 0) {
            continue;
