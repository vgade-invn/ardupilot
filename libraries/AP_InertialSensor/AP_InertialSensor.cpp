...
const AP_Param::GroupInfo AP_InertialSensor::var_info[] = {
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO("GYR_ID", 27, AP_InertialSensor, _gyro_id[0], 0),
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO("GYR2_ID", 28, AP_InertialSensor, _gyro_id[1], 0),
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO("GYR3_ID", 29, AP_InertialSensor, _gyro_id[2], 0),
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO("ACC_ID", 30, AP_InertialSensor, _accel_id[0], 0),
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO("ACC2_ID", 31, AP_InertialSensor, _accel_id[1], 0),
    ...
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO("ACC3_ID", 32, AP_InertialSensor, _accel_id[2], 0),
                                         ...
                                         uint32_t id)
{
    ...
    if (saved && (uint32_t)_gyro_id[_gyro_count] != id) {
        // inconsistent gyro id
        _gyro_cal_ok[_gyro_count] = false;
    }

    _gyro_id[_gyro_count].set((int32_t) id);

#if HAL_BOARD == HAL_BOARD_PX4 || HAL_BOARD == HAL_BOARD_VRBRAIN || HAL_BOARD == HAL_BOARD_SITL
    if (!saved) {
                                          ...
                                          uint32_t id)
{
    ...
    if (saved && (uint32_t)_accel_id[_accel_count] != id) {
