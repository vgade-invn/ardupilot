...
void AP_InertialSensor_SITL::generate_accel()
{
    ...
    for (uint8_t j = 0; j < nsamples; j++) {
        ...
        float accel_noise = sitl->accel_noise[accel_instance]>0?0.01:0;
...
void AP_InertialSensor_SITL::generate_gyro()
{
    ...
    for (uint8_t j = 0; j < nsamples; j++) {
        float p = radians(sitl->state.rollRate);
        float q = radians(sitl->state.pitchRate);
        float r = radians(sitl->state.yawRate);
#if 0 // turn off most gyro errors to help with INS error debug
        p += gyro_drift();
        q += gyro_drift();
        r += gyro_drift();
        // minimum gyro noise is less than 1 bit
        float gyro_noise = sitl->gyro_noise[gyro_instance]>0?ToRad(0.04f):0;
        float noise_variation = 0.05f;
        // this smears the individual motor peaks somewhat emulating physical motors
        float freq_variation = 0.12f;
        // add in sensor noise
