...
public:
    ...
    // opposed to an IMU estimate
    bool airspeed_sensor_enabled(int8_t airspeed_index) const {
        return airspeed_index >= 0 && _airspeed != nullptr && _airspeed->use(airspeed_index) && _airspeed->healthy(airspeed_index);
    }
